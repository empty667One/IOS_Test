...
//                
//            }
            let scanResultVC = ScanResultVC()
            scanResultVC.scanBarCode = stringValue
    ...
    func setUpCamera() {
        if checkCameraAvaliable() {
            if checkCameraAuthorise() {
                ...
                arrays.addObject(AVMetadataObjectTypeQRCode)
                arrays.addObject(AVMetadataObjectTypeEAN13Code)
                arrays.addObject(AVMetadataObjectTypeEAN8Code)
                arrays.addObject(AVMetadataObjectTypeCode128Code)
                metaDataOutput.metadataObjectTypes = arrays as [AnyObject]
