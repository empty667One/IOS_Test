...
struct DataService {
    ...
    private enum ResourcePath: CustomStringConvertible {
        ...
        var description: String {
            ...
            case .WXPayPrePay(let id):
                return "/wxpay/prepay/\(id)"
    ...
    static func reserveTour(productID productID: Int, date:String,
        response: (MyOrder?,NSError?) -> ()) {
            let urlString = WeixinNotifyURL + ResourcePath.Purchase.description
            ...
            request(.POST, urlString, parameters: parameters).response { (_, _, data, error) -> Void in
                ...
                if error == nil {
                    if let json = JSONType.fromData(data!) {
    ...
    static func wxPrePay(orderID:Int, response: (WXPrePay?,NSError?) -> ()) {
        ...
        let urlString = WeixinNotifyURL + path.description
